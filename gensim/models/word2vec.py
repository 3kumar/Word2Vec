...
try:
    ...
    def train_batch_cbow(model, sentences, alpha, work=None, neu1=None):
        ...
        Here the (word,conext) pairs are formed depending on the context window, for multiple context
        words the mean of projection is calculated if cbow_mean>0.
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    if model.negative:
        ...
        l2b = model.syn1neg[word_indices]  # 2d matrix, typex layer1_size
...
class Word2Vec(utils.SaveLoad):
    ...
    def build_vocab(self, sentences,update=False, keep_raw_vocab=False, trim_rule=None):
        """
        Build vocabulary from a sequence of sentences (can be a once-only generator stream).
        Each sentence must be a list of unicode strings.

        """
        self.scan_vocab(sentences,update=update, trim_rule=trim_rule)  # initial survey
        self.scale_vocab(keep_raw_vocab=keep_raw_vocab, update=update,trim_rule=trim_rule)  # trim by min_count & precalculate downsampling
        self.finalize_vocab(update=update)  # build tables & arrays

    def scan_vocab(self, sentences,update=False, progress_per=10000, trim_rule=None):
        ...
        for sentence_no, sentence in enumerate(sentences):
            ...
            '''prune the sentences only when creating vocab for the first time, if it is done
               always then the words added at the end will have low frequency and will be pruned in case max_vocab_size is excedded
            '''
            if not update:
    ...
    def scale_vocab(self, min_count=None, sample=None,update=False, dry_run=False, keep_raw_vocab=False, trim_rule=None):
        """
        ...
        for word, v in iteritems(self.raw_vocab):
            if (not update) or (update and not word in self.vocab):
    ...
    def finalize_vocab(self,update=False):
        ...
        if not self.index2word:
            self.scale_vocab(update=False) # do not change this to true
    ...
    def update_weights(self):
        ...
        new_syn0 = empty((len(self.vocab), self.vector_size), dtype=REAL)
        if self.hs:
            new_syn1 = zeros((len(self.vocab), self.layer1_size), dtype=REAL)
        if self.negative:
            new_syn1neg = zeros((len(self.vocab), self.layer1_size), dtype=REAL)


        # copy the weights that are already learned
        for i in xrange(0, len(self.syn0)):
            new_syn0[i] = deepcopy(self.syn0[i])
            if self.hs:
                new_syn1[i] = deepcopy(self.syn1[i])
            if self.negative:
                new_syn1neg[i] = deepcopy(self.syn1neg[i])

        # randomize the remaining words for input to hidden layer
        for i in xrange(len(self.vocab), len(new_syn0)):
            # construct deterministic seed from word AND seed argument
            self.syn0[i] = self.seeded_vector(self.index2word[i] + str(self.seed))

        # assign back the new weights to the model
        self.syn0 = deepcopy(new_syn0)
        if self.hs:
            self.syn1 = deepcopy(new_syn1)
        if self.negative:
            self.syn1neg = deepcopy(new_syn1neg)
